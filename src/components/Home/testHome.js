...
import { 
  FaBoxOpen, FaShoppingCart, FaMoneyBillWave, FaBolt, 
  FaBell, FaCalendarAlt, FaChartLine, FaSearch 
} from 'react-icons/fa';
      ...
      <div>
        ...
        <ul className="text-sm space-y-1">
          {recentItems.map((item, index) => (
            <li key={index} className="truncate">• {item.name || item.title || item.item || item.description}</li>
          ))}
        </ul>
      </div>
    )}
  ...
  useEffect(() => {
    const fetchSummaryData = async () => {
      try {
        ...
        const inventoryQuery = query(inventoryRef, orderBy("timestamp", "desc"), limit(3));
        ...
        const consumptionQuery = query(consumptionRef, orderBy("timestamp", "desc"), limit(3));
        ...
        const financeQuery = query(financeRef, orderBy("timestamp", "desc"), limit(3));
        ...
        const utilityQuery = query(utilityRef, orderBy("timestamp", "desc"), limit(3));
        ...
        const expiring = expirySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="lg:ml-64 p-6">
        ...
        <header className="mb-8">
          ...
          <p className="text-gray-600 mt-1">Welcome to Home Stock Mate, your household inventory management system</p>
        </header>
                ...
                {searchResults.map(item => (
                  <div key={item.id} className="py-2 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{item.type} • Quantity: {item.quantity}</p>
                    </div>
                    <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <section className="bg-white rounded-lg shadow-md p-5">
            ...
            {expiringItems.length > 0 ? (
                ...
                {expiringItems.map(item => (
                  <div key={item.id} className="py-3">
                    <div className="flex justify-between">
                      <p className="font-medium">{item.item}</p>
                      <span className="text-red-500 text-sm">{formatDate(item.expiryDate)}</span>
                    </div>
              ...
              <p className="text-gray-500 py-4">No items expiring in the next 7 days</p>
            )}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {[...summaryData.consumption.recentItems, ...summaryData.inventory.recentItems]
                  ...
                  .map((item, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {item.action || (summaryData.inventory.recentItems.some(i => i.id === item.id) ? 'Added' : 'Consumed')}
                      </td>
                    </tr>
                  ))}
              </tbody>
        ...
        <footer className="mt-16 text-center text-gray-500 text-sm">
          ...
          <p className="mt-1">Simplifying the way you track and manage household items</p>
        </footer>
      </div>
    </div>
  );
